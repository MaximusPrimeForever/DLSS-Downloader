...
def swap_dlss(should_list_versions: bool,
    ...
    if not game_dir_path and not download_version:
    ...
    if should_download:
        cwd = os.getcwd()
        dlss_path = Path(cwd, DLSS_FILENAME)

        print(f"Writing DLSS {selected_version_str} to {cwd}")
        with open(dlss_path, 'wb') as f:
